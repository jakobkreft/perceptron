...
class NeuralNetwork:
    def __init__(self):
        ...
        self.loss = None
        self.optimizer = None
        self.loss_history = []  # Shrani za vazualizacijo
    ...
    def train(self, X_train, Y_train, epochs, batch_size):
        for epoch in range(epochs):
            ...
            self.loss_history.append(epoch_loss)  # Store loss for this epoch

            val_loss, val_accuracy = self.validate(X_test, Y_test)
            self.val_accuracy_history.append(val_accuracy)  # Store validation accuracy
...
model.set(CategoricalCrossentropyLoss(), MomentumOptimizer(learning_rate=0.01, momentum=0.0))
...
# Izris grafa izgube pri učenju v ločenem oknu
plt.figure(figsize=(6, 5))
plt.plot(model.loss_history, label='Napaka pri učenju')
plt.title('Napaka pri učenju')
plt.xlabel('Epoha')
plt.ylabel('Napaka')
plt.legend()
plt.show()  # To bo prikazalo prvi graf

# Izris grafa točnosti pri validaciji v ločenem oknu
plt.figure(figsize=(6, 5))
plt.plot(model.val_accuracy_history, label='Uspešnost pri validaciji')
plt.title('Uspešnost pri validaciji')
plt.xlabel('Epoha')
plt.ylabel('Uspešnost')
plt.legend()
plt.show()  # To bo prikazalo drugi graf
